...
yellow_2017_summary <- fread("/Users/priscilla/Desktop/Honors Thesis/data/yellow_2017_summary.csv")
```
...
```{r tip_pickup, message=FALSE, echo=FALSE}
...
```{r num-trip, fig.cap="Number of Pick-ups in Each Taxi Zone", fig.align='center', echo=FALSE, fig.width=6, fig.height=6, echo=FALSE}
knitr::include_graphics("figure/num_trip.png")
```
...
```{r tip_pickup_zone, message=FALSE, echo=FALSE}
...
Most yellow cab pick-ups occur in Manhattan. If we focus on the pick-up zones that have more than 30 trips per day or 10950 per year, we will observe that many taxi pick-up zones with highest percent tips are not necessarily the ones with the highest number of pick-ups. 
```{r}
...
```{r pickup_zone_30, echo=FALSE}
kable(pickup_zone_30[1:10, ], caption = "Ten taxi pick-up zones with the highest percent tip with threshold equals to 900")
```
...
```{r pickup_zone_3000, echo=FALSE}
kable(pickup_zone_3000[1:10,], caption = "Ten taxi pick-up zones with the highest percent tip with threshold equals to 90000")
```
