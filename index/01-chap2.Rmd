...
In addition, the trip level NYC Uber data only covers two periods, from April to September 2014 and from January to June 2015. However, the summary reports cover weekly-aggregated data from 2015 to the most recent week [@datauber] (Figure 2.1).
...
```
Visualizations similar to Figure \@ref(fig:zonemap) can be generated with the shapefile.
...
Uber trip record data is static and small, so we decided to only give users the options to download either all data from April to September, 2014 or all Uber trip records from January to June, 2015 at once. We do not give users the option to download Uber data by month, since the total size of all trip-level data from 2014 and 2015 are small.
...
`init.mysql` is written under `nyctaxi` to help users to set up five basic table structures for MySQL database. `yellow_old` is created for Yellow Taxi data that are prior to July 2016, and `yellow` is created for data later than June 2016. There are two tables created for yellow taxi data because NYC TLC changed the variables included in yellow taxi datasets in July 2016. Table `green`, `uber`, and `lyft` are also initiated for green taxi, Uber and Lyft data. 
