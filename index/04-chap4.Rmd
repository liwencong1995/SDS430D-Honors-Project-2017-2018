...
## How long does it take passengers to get to JFK, La Guardia, and Newark Airports from anywhere in New York City? When is the best time to travel in order to avoid the traffic?
...
There are two commonly known bad weather consitions, rainy and snowy days. Let's first focus on snowstorm. On March 14th, 2017, a snow storm brought seven inches of snow to New York City.
**Yellow Taxi**
```{r, echo=FALSE}
yellow_weekly_data %>%
...
People living in New York might have noticed that it is hard to find a taxi on the street when it rains. Economists have studied this phenomena for a long time, and an analysis studied the correlationship between taxi movement and hourly rainfall data in Central Park from 2009 to 2013 has found that there is no significant correlation between a driver's hourly wage and rain in the city, which implies that drivers don't earn more when it's raining [@citylab]. 

I got access to the 2017 daily Central Park weather data from the National Climatic Data Center by submitting a Climate Data Online request (Appendix D) to National Centers for Environmental Information [@noaa], and joined it to the 2017 taxi data to study relationship between rainfall and taxi rides. 
...
```{r, echo=FALSE}
weather_day_1 <- weather_day %>%
  mutate(`Previous Week` = as.Date(`Pickup Start Date`)-7) %>%
  mutate(`Pickup Start Date` = as.character(`Pickup Start Date`),
         `Previous Week` = as.character(`Previous Week`))
weather_day <- weather_day %>%
  mutate(`Pickup Start Date` = as.character(`Pickup Start Date`))
weather_day_1 <- weather_day_1 %>%
  left_join(weather_day, by = c("Previous Week"="Pickup Start Date")) 

# rain_week <- weather_day_1 %>%
#   mutate(diff = `Weekly Rainfall.x` - `Weekly Rainfall.y`) %>%
#   arrange(desc(diff))

weather_yellow <- weather_day_1 %>%
  left_join(yellow_weekly_data, by = "Pickup Start Date") %>%
  rename(`Current Week Dispatched Trips` = `Total Dispatched Trips`)

weather_yellow_1 <- weather_yellow %>%
  left_join(yellow_weekly_data, by = c("Previous Week" = "Pickup Start Date")) %>%
  rename(`Last Week Dispatched Trips` = `Total Dispatched Trips`)

weather_yellow_1 <- weather_yellow_1 %>%
  mutate(`Percetage Change In Number of Trips` = ((`Current Week Dispatched Trips`-`Last Week Dispatched Trips`)/`Last Week Dispatched Trips`) *100)
```

```{r, message=FALSE, echo=FALSE}
kable(weather_day_1[1:10,],
      ...
      booktabs = TRUE)
```
First, we generate a list of total amount of daily rainfall in New York City and we pick the 10 weeks that have the most rainfall in 2017. We then find the weekly total number of dispatched yellow taxi trips of the 4 weeks with the srtat date listed above. We also need to add the weekly total number of dispatched Uber trips of the 10 weeks with the most rainfall.

```{r, echo=FALSE}
uber_weekly_data <- uber_weekly_data %>%
  select(`Pickup Start Date`, `Total Dispatched Trips`) %>%
  mutate(month = substr(`Pickup Start Date`, 1, 2),
         day = substr(`Pickup Start Date`, 4, 5)) %>%
  mutate(`Pickup Start Date` = paste0("2017-", month, "-", day)) %>%
  group_by(`Pickup Start Date`) %>%
  summarise(`Total Dispatched Trips` = sum(`Total Dispatched Trips`))

weather_uber <- weather_day_1 %>%
  left_join(uber_weekly_data, by = "Pickup Start Date") %>%
  rename(`Current Week Dispatched Trips` = `Total Dispatched Trips`)

weather_uber_1 <- weather_uber %>%
  left_join(uber_weekly_data, by = c("Previous Week" = "Pickup Start Date")) %>%
  rename(`Last Week Dispatched Trips` = `Total Dispatched Trips`)

weather_uber_1 <- weather_uber_1 %>%
  mutate(`Percetage Change In Number of Trips` = ((`Current Week Dispatched Trips`-`Last Week Dispatched Trips`)/`Last Week Dispatched Trips`) *100)

weather_day_1$uber <- weather_uber_1$`Percetage Change In Number of Trips`
weather_day_1$yellow <- weather_yellow_1$`Percetage Change In Number of Trips`
```

```{r, message=FALSE, echo=FALSE}
kable(weather_day_1[1:10, c(1:2, 5:6)],
      caption = "10 weeks that have the most rainfall in 2017 and the total number of dispatched yellow taxi trips in those weeks",
      format = "latex",
      align = 'c', 
      escape = TRUE,
