# New York City Taxi Driver {#chapter2}
...
```{r tip-individual, fig.cap="this is my caption"}
library(ggplot2)
...
We first calculate the average percent tip paid for each pick-up zone. Here is a list of pick-up zones with more than 1000 trip per month along with their average percent tip:
```{r}
tip_pickup <- yellow_2016.08_tip %>%
  ...
  filter(Zone != "Unknown")
tip_pickup %>% filter(num_trips > 1000) %>% head(10)
```
...
Let's fist take a look at which pick-up zones have the highest number of pickups. We create a heat map to visulizae the number of trip for each pick-up zones on a map of New York City Taxi Zones.
...
```
It's obbivous that Manhattan, La Guardia Airport, and JKF Airport have the most number of pick-ups.
...
# library(knitr)
# kable(pickup_zone_1000[1:10,2:6], caption = "Taxi zone with the highest tip percent, threshold = 1000")
```
