...
library(lubridate)
to_jfk_1 <- fread("/Users/priscilla/Desktop/Honors Thesis/mysql/csv sum/to_jfk.csv")
...
to_jfk <- fread("/Users/priscilla/Desktop/Honors Thesis/mysql/csv sum/to_jfk_small.csv")
...
knitr::include_graphics("figure/to_jfk_fare_vis.png", dpi = 200 )
```
...
We computed the average fare paid by passengers for trips going from each taxi zone in Manhattan to JFK Airport in Table \@ref(tab:to_jfk_zone_above).
...
```{r, echo=FALSE}
meanfare <- mean(to_jfk_zone$avg_est_diff)
meanfare
```
...
kable(from_jfk_zone[1:5,c(5,6,2:4)], 
      ...
      booktabs = TRUE)
```
Table \@ref(tab:from_jfk_zone) shows that Times Square is the most popular destination for passengers coming from the JFK Airport in 2017!
...
knitr::include_graphics("figure/from_jfk_num_trips.png", dpi = 200 )
```
...
```{r, echo=FALSE}
total_trip <- from_jfk_zone[2,2]/(from_jfk_zone[1,2] + from_jfk_zone[2,2])
```
...
```{r, message=FALSE, echo=FALSE}
kable(from_jfk_zone[1:10,c(5,6,2:4)],
...
# from_jfk_fare_est <- leaflet(data = from_jfk_fare) %>%
#   addProviderTiles(providers$OpenStreetMap) %>%
#   addPolygons(fillColor = ~reds(avg_est_fare),
#               fillOpacity = 0.6,
#               weight = 1,
...
#   setView(lat = 40.7128, lng = -74.0060, zoom = 11)
# 
# saveWidget(from_jfk_fare_est, file = "/Users/priscilla/Desktop/Honors Thesis/writing/figures/from_jfk_fare_est.html")
# URL <- "/Users/priscilla/Desktop/Honors Thesis/writing/figures/from_jfk_fare_est.html"
# webshot(URL, "figure/from_jfk_fare_est.png", cliprect = "viewport")
```

```{r from-jfk-fare-vis, fig.cap="Zones that cost more than the 52 US Dollar flat rate", fig.align='center', echo=FALSE}
knitr::include_graphics("figure/from_jfk_fare_vis.png", dpi = 200 )
```
...
from_jfk_zone_1 <- from_jfk_zone %>%
  filter(avg_duration > 10) %>%
  mutate(hourly_wage = avg_fare/(avg_duration/60))
mean_from_jfk_wage <- mean(from_jfk_zone_1$hourly_wage)
mean_from_jfk_wage
```
...
In conclusion, taxi drivers' hourly wages are higher when they drive from Manhattan to JFK Airport. On their way back, they make on average `mean_from_jfk_wage` dollar per hour, which is much higher than the average hourly wage of yellow cab drivers. Taxicab drivers feel happy when they pick up passengers who want to go to JFK Airport in Manhattan. Even though \$52 is lower than the average amount of fare that taxi drivers would have made without the flat rate, it still induces an higher than average hourly wage, so it does not disincentive drivers. Therefore, the \$52 flat rate is appropriate. 
