...
```{r to_jkf_fare_vis, fig.cap="Estmated fare amount from the each pick-up zone to JFK Airport", fig.align='center', echo=FALSE, fig.width=6, fig.height=6}
knitr::include_graphics("figure/to_jkf_fare_vis.png")
```
...
```{r num-to_jfk_fare_above_vis, fig.cap="Zones that cost more than the \$52 flat rate", fig.align='center', echo=FALSE, fig.width=6, fig.height=6}
knitr::include_graphics("figure/to_jfk_fare_above_vis.png")
```
...
```
On average, taxi drivers would get paid for \$55.77 for a trip from the JFK Airport to any taxi zone in Manhattan. What are the most popular drop-off zones for yellow taxis from JFK Airport?
```{r, echo=FALSE}
...
```{r from_jkf_zone, message=FALSE, echo=FALSE}
kable(from_jkf_zone[1:10,], caption = "Ten most popular destinations in Manhattan")
```
