...
\StringTok{  }\KeywordTok{filter}\NormalTok{(num_trips }\OperatorTok{>=}\StringTok{ }\NormalTok{(}\DecValTok{24}\OperatorTok{*}\DecValTok{365}\NormalTok{)) }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(num_trips }\OperatorTok{>=}\StringTok{ }\NormalTok{(}\DecValTok{365}\OperatorTok{*}\DecValTok{24}\OperatorTok{*}\DecValTok{60}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{arrange}\NormalTok{(}\KeywordTok{desc}\NormalTok{(avg_tip))}
\end{Highlighting}
\end{Shaded}
\begin{table}
...
\caption{\label{tab:unnamed-chunk-74}10 weeks that have the most rainfall in 2017}
\centering
\begin{tabular}[t]{ccc}
\toprule
...
\caption{\label{tab:unnamed-chunk-76}10 weeks that have the most rainfall in 2017 and the total number of dispatched yellow taxi trips in those weeks}
\centering
\begin{tabular}[t]{ccccc}
\toprule
...
\caption{\label{tab:unnamed-chunk-78}10 weeks that have the most rainfall in 2017 and the total number of dispatched Uber trips in those weeks}
\centering
\begin{tabular}[t]{ccccc}
\toprule
Pickup Date & Dispatched Trips & Last Week Date & Last Week Trips & \%Change Trips\\
...
\caption{\label{tab:unnamed-chunk-80}The percentage change in total number of dispatched trips comparing to the previous weeks of yellow taxi and Uber}
\centering
\begin{tabular}[t]{ccccc}
\toprule
...
instead. NYC TLC could modify the rate of fare on rainy or snowy days to
incentive taxicab drivers to pick up more trips in order to make taking
a street hail vechcle on average more affordable on rainy days for
passengers.
...
Read 51.9% of 577783 rows
Read 64.0% of 577783 rows
Read 96.9% of 577783 rows
Read 577783 rows and 20 (of 20) columns from 0.064 GB file in 00:00:05
\end{verbatim}
\begin{verbatim}

Read 0.0% of 1491842 rows
Read 5.4% of 1491842 rows
Read 22.8% of 1491842 rows
Read 38.9% of 1491842 rows
Read 53.0% of 1491842 rows
Read 55.0% of 1491842 rows
Read 72.4% of 1491842 rows
Read 85.1% of 1491842 rows
Read 1491842 rows and 20 (of 20) columns from 0.164 GB file in 00:00:15
\end{verbatim}
...
\caption{\label{tab:unnamed-chunk-91}Ten pick-up zones with the highest avergae fare from Manhattan to JKF Airport}
\centering
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{meanfare <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(to_jkf_zone}\OperatorTok{$}\NormalTok{avg_est_diff)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tab:unnamed-chunk-97}Ten most popular destinations in Manhattan}
\centering
\begin{tabular}[t]{cccc}
\toprule
