...
\StringTok{   }\KeywordTok{etl_extract}\NormalTok{(}\DataTypeTok{years =} \DecValTok{2017}\NormalTok{, }\DataTypeTok{months =} \DecValTok{1}\OperatorTok{:}\DecValTok{12}\NormalTok{, }\DataTypeTok{type =} \KeywordTok{c}\NormalTok{(}\StringTok{"yellow"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{taxi }\OperatorTok{%>%}\StringTok{ }
\StringTok{   }\KeywordTok{etl_extract}\NormalTok{(}\DataTypeTok{years =} \DecValTok{2017}\NormalTok{, }\DataTypeTok{months =} \DecValTok{1}\OperatorTok{:}\DecValTok{12}\NormalTok{, }\DataTypeTok{type =} \KeywordTok{c}\NormalTok{(}\StringTok{"uber"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{taxi }\OperatorTok{%>%}\StringTok{ }
\StringTok{   }\KeywordTok{etl_extract}\NormalTok{(}\DataTypeTok{years =} \DecValTok{2017}\NormalTok{, }\DataTypeTok{months =} \DecValTok{1}\OperatorTok{:}\DecValTok{12}\NormalTok{, }\DataTypeTok{type =} \KeywordTok{c}\NormalTok{(}\StringTok{"lyft"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
formats, and it utlizes different data cleaning techiniques when it
...
from being mondified or destroyed. Users are allows to specify the month
...
\begin{Highlighting}[]
\NormalTok{taxi }\OperatorTok{%>%}\StringTok{ }
\StringTok{   }\KeywordTok{etl_transform}\NormalTok{(}\DataTypeTok{years =} \DecValTok{2017}\NormalTok{, }\DataTypeTok{months =} \DecValTok{1}\OperatorTok{:}\DecValTok{12}\NormalTok{, }\DataTypeTok{type =} \KeywordTok{c}\NormalTok{(}\StringTok{"yellow"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
columns attanched to the right-most column. These blank row and solumn
causes problems in the later stage when users want to load data into SQL
database. In order to get Green Taxi data ready for the \texttt{load}
phase, we used \texttt{system()} to invoke the OS command specified to
...
\texttt{etl\_transform}. \texttt{data.table} only takes half of the time
to read from and write into datasets comparing to \texttt{readr}.
...
\begin{Highlighting}[]
\NormalTok{taxi }\OperatorTok{%>%}\StringTok{ }
\StringTok{   }\KeywordTok{etl_load}\NormalTok{(}\DataTypeTok{years =} \DecValTok{2017}\NormalTok{, }\DataTypeTok{months =} \DecValTok{1}\OperatorTok{:}\DecValTok{12}\NormalTok{, }\DataTypeTok{type =} \KeywordTok{c}\NormalTok{(}\StringTok{"yellow"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\section{New Yoek City Taxicab and E-hail Services
Summary}\label{new-yoek-city-taxicab-and-e-hail-services-summary}

Below is a summary of total number of trips made by all 4 types of
transporations that are available to users from 2014 to 2016. In order
to generate this summary, I combined trip-level yellow and green taxi
data from TLC trip data website and weekly Uber and Lyft data from NYC
OpenData.
\begin{figure}
...
\caption{\label{tab:unnamed-chunk-37}Ten taxi pick-up zones with the highest average tip without any threshold in Janaury, 2017}
\centering
...
\caption{\label{tab:unnamed-chunk-41}Ten taxi zones with the highest number of pick-ups}
\centering
...
\caption{\label{tab:unnamed-chunk-43}Ten taxi pick-up zones with the highest percent tip (taxi zones has at least 30 pick-up per day)}
\centering
\begin{tabular}[t]{r|r|r|r|r|l|l}
\hline
PULocationID & avg\_tip & num\_trips & avg\_dis & avg\_duration & Borough & Zone\\
...
\caption{\label{tab:unnamed-chunk-45}Ten taxi pick-up zones with the highest percent tip  (taxi zones has at least 3000 pick-up per day)}
\centering
\begin{tabular}[t]{r|r|r|r|r|l|l}
\hline
PULocationID & avg\_tip & num\_trips & avg\_dis & avg\_duration & Borough & Zone\\
...
\caption{\label{tab:unnamed-chunk-55}Average number of minutes it takes from Alphabet City, Manhattan to JFK Airport during different hours}
\centering
...
\caption{\label{tab:unnamed-chunk-60}Uber 2017 Weekly Total Dispatched Trips}
\centering
...
\caption{\label{tab:unnamed-chunk-63}Yellow Taxi 2017 Weekly Total Dispatched Trips}
\centering
\begin{tabular}[t]{l|r}
\hline
...
Read 26.6% of 1581568 rows
Read 37.3% of 1581568 rows
Read 46.2% of 1581568 rows
...
\caption{\label{tab:unnamed-chunk-74}Ten pick-up zones with the highest avergae fare from Manhattan to JKF Airport}
\centering
...
\caption{\label{tab:unnamed-chunk-80}Ten most popular destinations in Manhattan}
\centering
\begin{tabular}[t]{r|r|r|r|l|l}
\hline
LocationID & num\_trips & avg\_est\_fare & avg\_est\_diff & Borough & Zone\\
